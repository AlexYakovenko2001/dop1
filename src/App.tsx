...
function App() {
    ...
    setTodo(todo.map((el, index) => index === todolistId ? {...el, tasks: el.tasks.filter(t => t.taskId !== id)} : el))
    }

    function addTask(title: string, todolistId: number) {
        const task = {taskId: v1(), title: title, isDone: false};
        setTodo(todo.map((el, index) => index === todolistId ? {...el, tasks: [task, ...el.tasks]} : el))
    }
